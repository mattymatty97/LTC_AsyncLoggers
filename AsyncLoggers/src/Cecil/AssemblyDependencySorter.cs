...
public static class AssemblyDependencySorter
{
    public static IEnumerable<string> SortAssembliesByDependency(IEnumerable<string> assemblies)
    {
        ...
        foreach (var path in assemblies)
            using (var assemblyDefinition = AssemblyDefinition.ReadAssembly(path,
                       new ReaderParameters { ReadSymbols = false, ReadingMode = ReadingMode.Deferred }))
            {
    ...
    private static bool DetectCycles(string node, Dictionary<string, List<string>> graph, HashSet<string> visited,
        HashSet<string> visiting, HashSet<string> cyclicNodes)
    {
    ...
    private static void TopologicalSortVisit(string node, Dictionary<string, List<string>> graph,
        HashSet<string> visited, List<string> sorted)
    {
        if (!visited.Contains(node))
        {
            visited.Add(node);
