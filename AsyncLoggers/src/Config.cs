...
internal static class XmlConfig
{
    ...
    private const string UsageComment =
        ...
        4. **Delay Attribute** (only applicable if `status="BepInEx"`):
           - You can add an optional `delay` attribute to throttle logging. 
           - The `delay` attribute should be an integer value representing milliseconds.
           - Example: `<LogCall status="BepInEx" delay="500">LogError("Error message")</LogCall>`
    ...
    internal static void InitLogConfig()
    {
        ...
        if(configFile.DocumentElement == null)
        {
            ...
            var attr = configFile.CreateAttribute("default_status");
        ...
        if (Enum.TryParse(configFile.DocumentElement!.GetAttribute("default_state"), out CallStatus defaultStatus) && defaultStatus != CallStatus.Default)
        {
